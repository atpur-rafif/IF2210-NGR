...
void PlayerView::start(Player &player) {
  while (true) {
    ...
    function<string(string)> fn = [](string input) { return input; };
...
void PlayerView::eat(Player &player) {
  ...
  function<void(string, optional<shared_ptr<Item>> &)> fn =
      [](string, optional<shared_ptr<Item>> &opt) {
        if (!opt.has_value())
          throw PromptException("Tidak bisa memakan petak kosong");

        shared_ptr<ProductItem> product =
            dynamic_pointer_cast<ProductItem>(opt.value());
        if (product == nullptr)
  ...
  string location =
      CLI::promptStorageLocation("Petak untuk dimakan: ", player.inventory, fn);
  player.eat(location);
}
...
void PlayerView::runPlayerCommand(Player &player, string command) {
  if (command == "BELI")
    ShopView::buyItem(player);
  else if (command == "JUAL")
    ShopView::sellItem(player);
    ...
    throw CommandNotFoundCLIException();
}
